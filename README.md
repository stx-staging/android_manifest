...
## Repo Init ##
```bash
repo init -u https://github.com/stx-staging/android_manifest.git -b bp3a
```
## Sync Source ##
```bash
...
```
#### For projects using this manifest, `<RELEASE>` is `bp3a`.
### Submitting Patches ###
...
* `<branch>` - The git branch your change is based on; for projects using this manifest, it is `bp3a`
